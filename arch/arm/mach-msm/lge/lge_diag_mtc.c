...
DIAG_MTC_F_rsp_type* mtc_capture_screen(DIAG_MTC_F_req_type *pReq)
{
	...
	packet_len = rsp_len - (sizeof(unsigned short) * MTC_SCRN_BUF_SIZE_MAX) + ((x_end-x_start)*((y_end-y_start)*sizeof(unsigned short)));
	...
	bmp_size = read_framebuffer(tmp_img_block);
...
static ssize_t read_framebuffer(byte* pBuf)
{
  	...
  	read_size = phMscd_Filp->f_op->read(phMscd_Filp, pBuf, MTC_SCRN_BUF_SIZE_MAX*sizeof(unsigned short), &phMscd_Filp->f_pos);
